if (typeof window.MenuAccordion === 'undefined') {
	window.MenuAccordion = class {
		constructor() {
			(function ($) {
				...
				$.fn.mgaccordion = function (options) {
							...
							if ($this.children('ul').length) {
								$this.addClass('dropdown')
									...
									.bind('click', function (e) {
										//controllo l'esistenza della classe per capire se è il menù collassato a sinistra
										if (!$("#mainmenu").hasClass("vertical")) {
											e.preventDefault();
											if (leaveOpen === false) {
												closeOther($(this));
											}
								...
								if (theme === 'tree') {
									...
									$this.children('a').append('<span class="toggler"><i class="fas fa-chevron-down"></i></i></span>');
								}
							}
						});
					...
					var setIcons = function () {
						if (settings.theme === 'tree') {
							...
							openIcon = '<span class="toggler"><i class="fas fa-chevron-down"></i></i> </span>';
							closeIcon = '<span class="toggler"> <i class="fas fa-chevron-up"></i></i></span>';
						}
					}
	...
	let menuAccordion = new MenuAccordion();
}
/*
...
const loadMenuAccordion = () => {
	if (!$('.my-nav').hasClass('mg-accordion')) {
		$('.my-nav').mgaccordion();
	}
}