	...
	window.ContextMenu = class {
		...
		#setPosition(element) {
			...
			if (element) {
				...
				const bBottom = screenHeight >= menu.offsetTop + item.offsetTop + item.clientHeight + height;
				//codice nuovo e NON funzionante, ho ripristinato quello vecchio qui sopra che andava
				//const bBottom = screenHeight >= getIntValue(menu.style.top) + height;
				//console.log(bBottom, screenHeight, item.offsetTop, item.clientHeight, height)

				element.style.left = (bRight ? item.clientWidth : (-1 * width)) + 'px';
				element.style.top = (bBottom ? 0 : (-1 * (item.offsetTop - item.offsetHeight))) + 'px';
				//codice nuovo e NON funzionante, ho ripristinato quello vecchio qui sopra che andava
...
function contextMenuCreate(structure = [], init = {}) {
	...
	if (init?.target) {
			...
			if (target) {
				console.log(target);
				if (target.childNodes.length !== null && target.childNodes.length > 0) {
					if (target.childNodes[0].type !== undefined && target.childNodes[0].type !== null){
						console.log(target.childNodes[0].type);
						if (target.childNodes[0].type.toUpperCase() == 'CHECKBOX'){
							bOk=false;
						}
					}
				}
				
				if (target.classList.toString().includes('button') == 0) {
					...
					if (target.value && target.value.length > 0) {
							...
							if (!isLocked) {
									group.push({
											'icon': '<i class="fa-light fa-scissors"></i>',
											'title': 'Taglia',
											'isEnabled': true,
							...
							group.push({
									'icon': '<i class="fa-light fa-copy"></i>',
									'title': 'Copia',
									'isEnabled': true,
					...
					if (!(isLocked) && navigator.userAgent.search('Chrome')) {
							// Incolla
							group.push({
									'icon': '<i class="fa-light fa-paste"></i>',
									'title': 'Incolla',
									'isEnabled': true,
	...
	if (bOk){
		if (structure) {
			...
			setPosition(contextMenu.container, init?.position?.x, init?.position?.y, 5, 5);
		}
